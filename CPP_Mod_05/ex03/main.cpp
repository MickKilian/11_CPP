...
/*   Updated: 2024/10/23 19:20:52 by mbourgeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	//************************** TESTS_ON_INTERNS **************************
	std::cout << std::endl << YELLOW << "TESTS on INTERN" << RESET << std::endl;
	Intern intern1;
	AForm *form1_int;
	AForm *form2_int;
	try
	{
		form1_int = intern1.makeForm( "Robotomy request", "some guy" );
		delete form1_int;
	}
	catch(const std::exception& e)
	{
		std::cerr << "Intern cannot generate Robotomy request form because " << e.what() << ".\n";
	}
	try
	{
		form2_int = intern1.makeForm( "Free ride to the moon", "some guy" );
		delete form2_int;
	}
	catch(const std::exception& e)
	{
		std::cerr << "Intern cannot generate Free ride to the moon form because " << e.what() << ".\n";
	}
	std::cout << std::endl;
